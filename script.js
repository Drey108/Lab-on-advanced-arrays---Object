...
var mapNumbers = numbers.map(function(numbers){
  const changedNumber = parseInt(numbers); // Convert the string to a number
  return changedNumber * changedNumber; // Calculate the square
});
...
var newString = alphabets.reduce(function(x, y) {
  return x + y;
});
...
var firstEvenNumber = number.find(function(number) {
...
var indexOfNumber = ranks.findIndex(function(index){
  return index === 7;
})
...
var element = marks.some(function(mark) {
  return mark < 45;
});
console.log(element);
...
const positiveNumbers = positive.every(function(number) {
  return number > 0;
});
...
// 3. Using dot and bracket notation, get the output as "Mr Ajay Kumar travels by plane called Air123".
const result = `Mr ${person.name} travels by plane called Air123`;

console.log(result);

// 4. Change the email property of the person object to "ajaykumar@gmail.com".
